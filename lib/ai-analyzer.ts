...
- governmentWarning: Read the government warning block character-by-character from the image pixels. Return the text starting from "GOVERNMENT WARNING:" and stopping immediately after "health problems." — do NOT include any text that appears after "health problems." (e.g. "CONTAINS SULFITES", allergen statements, bottler addresses, barcodes). Return null if the block is blurry, too small to read, or if you cannot confirm every word from the pixels alone (see Section A below for detailed rules).
